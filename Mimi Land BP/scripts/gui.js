...
import { cleanNames, formatDimensionName, generateFantasyName, getSelectedNames, nanoid, readableCoords, sortOwners, toIsoStringWTZ, truncateText } from "./utils";
...
export class MimiLandGUI {
    static openMenu(player, selectedCoords, ParticleRunner) {
        ...
        const buttonOptions = [
            ...
            ["§lABOUT", "textures/ui/mimi/about1", () => this.handleAbout(player, selectedCoords)]
        ]
        ...
        userAreas.forEach((item, index) => {
            buttonOptions.unshift([`§l${item.name}§r\n §0§8Owned Land`, "textures/ui/icon_recipe_nature.png", () => this.handleLandDetails(player, item, selectedCoords)])
        })
    ...
    static handleCreateLand(player, selectedCoords, ParticleRunner) {
        ...
        createLandModal.show(player).then(({ canceled, formValues }) => {
            ...
            const landName = truncateText(formValues[0], 10); // Truncate land name to 20 characters
